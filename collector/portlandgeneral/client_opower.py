...
class OPowerApi:
    ...
    def _file_request(self, path: str, params: dict = None):
        return self._request_file('GET', self._api.api_endpoint + path, params)
    ...
    def _request_file(self, method: str, url: str, params: dict = None):
    ...
    # start_date should be in iso format YYYY-MM-DD
    # end_date should be in iso format YYYY-MM-DD
    # start and end dates will default from 2 days ago to yesterday if not specified
    def download_usage(self, opower_uuid: str, start_date: str = None, end_date: str = None) -> UtilityBillingWindows:
        today = date.today()
        yesterday = today - relativedelta(days=1)
        twodaysago = today - relativedelta(days=2)

        start_date = start_date if start_date is not None else twodaysago
        end_date = end_date if end_date is not None else yesterday
        
        response = self._file_request(
            f'/DataBrowser-v1/cws/utilities/pgn/customers/{opower_uuid}/usage_export/download?format=csv&startDate={start_date}&endDate={end_date}',
        )
